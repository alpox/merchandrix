...
function sortItemInfo(t, classId, desc)
	return sortBase(t, function(s1, s2)
		local v1 = select(classId, GetItemInfo(s1.id));
		local v2 = select(classId, GetItemInfo(s2.id));
...
function levelSort(t)
	return sortItemInfo(t, 4)
end

function levelSortDesc(t)
	return sortItemInfo(t, 4, true)
end
...
function sortedArmor(t)
	local ts = itemInvTypeSort(levelSortDesc(itemSubTypeSortDesc(itemNameSort(t))));
